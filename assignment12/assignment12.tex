...
\begin{abstract}
This document solves problem on representations of linear transformations
\end{abstract}
Download latex-tikz from
\begin{lstlisting}
https://github.com/shreeprasadbhat/matrix-theory/blob/master/assignment12/
\end{lstlisting}
%
\section{Problem}
...
\begin{align}\label{eq4}
	...
	\label{eq6}&= x_q \delta_{iq}\beta_p
\end{align}
Consider $T(\vec{x})$, from \eqref{eq1}
\begin{align}\label{eq7}
	T(\vec{x}) 
	&= \displaystyle \sum_{p=1}^m \displaystyle \sum_{q=1}^n A_{pq}E^{p,q}(\vec{x})
\end{align}
Substitute \eqref{eq6} in \eqref{eq7}
\begin{align}\label{eq8}
	T(\vec{x}) &= \displaystyle \sum_{p=1}^m \displaystyle \sum_{q=1}^n A_{pq}x_p \delta_{iq}\beta_q
\end{align}
\\
From \eqref{eq3}, $\delta_{iq}\beta_q$ is the transformation of basis of $V$ to $W$. Hence $T: V \rightarrow W$ is\\
\begin{align}
