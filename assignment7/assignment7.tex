...
\begin{align}\label{eq4}
	\implies\myvec{5 & 0 &-2}\myvec{a\\b\\c} = 0
\end{align}
Put $a=0$ and $b=1$ in \eqref{eq4}, $\implies c=0$\\
Put $a=1$ and $b=0$ in \eqref{eq4}, $\implies c=\frac{5}{2}$\\
...
Since given plane is parallel to Y-axis, we can use any point $\vec{P}$ on Y-axis to compute shortest distance. 
\begin{equation}\label{eq5}
	\text{Let,  } \vec{P} = \myvec{0\\0\\0}
\end{equation}
...
\end{align}
Equate $\vec{P}$ and $\vec{Q}$, and compute $\lambda_1, \lambda_2$ using $\textit{pseudoinverse}$ from $\textit{SVD}$ (since the given plane and Y-axis never intersect, pseudoinverse should give the points which are closest)
\begin{align}
...
\begin{align}
	...
	\label{eq15}&= \myvec{1 & 0\\0 & 1}\myvec{\frac{29}{4} & 0\\0 & 1}\myvec{1 & 0\\0 & 1}\\
...
\end{lstlisting}
Compute $\textit{SVD}$ of $\vec{M}$ from \eqref{eq13} and \eqref{eq16},
\begin{align}
...
Verifying solution to \eqref{eq10} with $\textit{least squares}$ method
\begin{align}
...
\begin{align}
	...
	\text{and }\lambda_2 &= 0\\
	\label{31}\implies\vec{x} &= \myvec{-\frac{14}{29}\\0}
\end{align}
